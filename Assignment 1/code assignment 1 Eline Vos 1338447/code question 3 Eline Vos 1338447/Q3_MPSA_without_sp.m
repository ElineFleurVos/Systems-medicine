function [Np, K_S, mean_dummies] = Q3_MPSA_without_sp(nr_dummies, nr_MPSA, sign)
%Does multi-Parametric Sensitivity Analysis with Try-Catch. All 25 model
%parameters are varied in the ensembling of parameter sets. 
...
% set initial values of dummy parameters to 1
for i = length(par0)+1:length(par0)+1+Nd 
     par0(i) = 1;
end
...
K_S = max(abs(Sa-Su));
mean_dummies = mean(K_S(Np+1:end)); %calculate mean KS statistic for dummies
