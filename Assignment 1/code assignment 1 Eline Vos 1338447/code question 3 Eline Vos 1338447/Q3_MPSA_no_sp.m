%Multi-Parametric Sensitivity Analysis with Try-Catch.
...
% Simulate default model for nominal parameter values par0 and calculate
% feature from selected outputs
feature0 = Q3_simulate(sample_person, par0);

Np = length(par0); %no. of parameters included in analysis
Nd = 5;            %no. of dummies
n_MPSA = 2500;     %no. of parameter ensembles / no. of Monte Carlo simulations
tot_params = Np+Nd;
...
for i = length(par0)+1:length(par0)+1+Nd %set dummie parameter values to 1
     par0(i) = 1;
end
...
for j = 1 : n_MPSA
    ...
    try %if simulation works, the sensitivity criterion for parameter set j gets a value 
        feature = Q3_simulate(sample_person, partemp);
...
threshold = nanmean(V);             %mean excluding NAN as threshold
...
%% Make plot with sensitivities for model parameters and dummies
X_param = 1:Np;
X_dummies = 1:Nd;
mean_dummies = mean(K_S(Np+1:end));
tiledlayout(1,2)

ax1 = nexttile;
scatter(X_param, K_S(1:Np))
yline(mean_dummies)
ylabel('sensititivity (KS-statistic)', 'FontSize',12)
xlabel('parameter #', 'FontSize',12)

ax2 = nexttile;
scatter(X_dummies, K_S(Np+1:end))
yline(mean_dummies)
%set(gca,'XTick',[26, 35])
xlabel('dummie #', 'FontSize',12)
sgtitle('sensitivities for model parameters and dummies')

linkaxes([ax1, ax2], 'y')

%%
save('mean_dummies_test.mat', "mean_dummies") 
