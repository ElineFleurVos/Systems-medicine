function out = Q5_simulate_meal_model(phenotypic_data,parameters,time,plot_col)
...
%% Generate figure
 ...
 s = csaps(sample_data.time_G, sample_data.glucose, 1); %cubic smoothing spline interpolation
 fnplt(s, 'r')
 hold on
 plot(sample_data.time_G, sample_data.glucose, 'k.', MarkerSize= 13)
 ylabel('glucose (mmol/l)');
 ...
 hold on
 %also plot the sample data 
 s = csaps(sample_data.time_I, sample_data.insulin, 1); %cubic smoothing spline interpolation
 fnplt(s, 'r')
 hold on
 plot(sample_data.time_I, sample_data.insulin, 'k.', MarkerSize= 13)
 ...
 hold on
 %also plot the sample data 
 s = csaps(sample_data.time_TG, sample_data.TG, 1); %cubic smoothing spline interpolation
 fnplt(s,'r')
 hold on
 plot(sample_data.time_TG, sample_data.TG, 'k.', MarkerSize= 13)
 ylabel('TG (mmol/l)');
 title('plasma TG')
 xlabel('time (mins)')
 ...
 hold on
 %also plot the sample data 
 s = csaps(sample_data.time_NEFA, sample_data.NEFA, 1); %cubic smoothing spline interpolation
 fnplt(s, 'r')
 hold on
 plot(sample_data.time_NEFA, sample_data.NEFA, 'k.', MarkerSize= 13)
 ylabel('NEFA (mmol/l)');
 title('plasma NEFA')
 xlabel('time (mins)')
