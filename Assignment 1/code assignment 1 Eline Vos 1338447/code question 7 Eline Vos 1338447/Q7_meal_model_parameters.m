function out = Q7_meal_model_parameters(p_opt,input_data)
...
out(5)  = p_opt(2);  %0.0424; %k5 rate constant for delayed insulin depedent uptake of glucose
out(6)  = 2.2975;    %k6 rate constant for stimulation of insulin production by the change of plasma glucose concentration (beta cell funtion)
...
out(11) = p_opt(3);  %sigma shape factor (appearance of meal)
out(12) = 13.2;      %Km michaelis-menten coefficient for glucose uptake
out(13) = p_opt(4);  %input_data.glucose(1);%G_b basal plasma glucose [mmol/l]
out(14) = p_opt(5);  %input_data.insulin(1); %I_PL/_b basal plasma glucose [microU/ml]
...
out(18) = p_opt(6);  %0.215;    %ATL_max maximum rate of ATL lipolysis in adipose tissues
out(19) = p_opt(7);  %0.0385;   %K_ATL michealis menten coeficient for ATL lipolysis of store TG in adipose tissue
out(20) = 0.0713;    %k12 rate constanst for uptake of NEFA into tissues (currently insulin indenpendent)
out(21) = p_opt(8);  %208;      %tau_LPL time delay for insulin stimulation of LPL lipolysis
out(22) = p_opt(9);  %0.0088;   %k13 - rate constant for stomach emptying TG(very slow)
...
out(25) = 0.0119;    %k16 basal secretion of TG from liver